...
def plot_training(env_name, save_path=None, smooth_window=100):
  ...
  def plot_progress(df, ax, metric, x_label='Step', x_value="time/total_timesteps"):
    ...
    smoothed = df[y_value].rolling(window=smooth_window, min_periods=1).mean()
    ax.plot(df.index, smoothed); ax.set_xlabel(x_label); ax.set_ylabel(y_label)
...
def plot_movement(initial_states, seeds, algorithms, colors, full, save_path, show_seeds=[42]):
  ...
  if full:
    ...
    ax2 = fig.add_subplot(1,4,2, projection='3d'); axs.append(ax2) # Target
...
def plot_trajectory(env_name, full=True, save_path=None):
  saved_model, checkpoint, env = ENVS(env_name)
  ...
  else: plot_movement(initial_states, seeds, algorithms, colors, full, save_path)
...
def render_videos(env_name, record_seeds=[42]):
  ...
  if 'Grid' in env_name: env_config['map_size'] = env.unwrapped.size[0] - 2
  CONFIG.set_eval_config(env=env, **env_config)
  video_path = VIDEO_PATH.joinpath(f"{env_name}/"); os.makedirs(video_path, exist_ok=True);
...
def plot(env_name: str, render: bool, training: bool):
    ...
    if training: plot_training(env_name, save_path=path.joinpath(f"{env_name}-Training.pdf"))
    ...
    plot_trajectory(env_name, full=False, save_path=path.joinpath(f"{env_name}-Heatmap"))
